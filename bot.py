...
UPDATE_CHANNEL = 'https://t.me/zemmedya'
...
def kill_process_tree(process_info):
    ...
    try:
        ...
        else: logger.error(f"Process object missing for {script_key}, and no log file.")
    except Exception as e:
...
def check_nodejs_installation():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.warning("⚠️ Node.js not found with 'node' command")
            
        # Try 'nodejs' command (some Linux distributions)
        result = subprocess.run(['nodejs', '--version'], 
                               capture_output=True, 
                               text=True, 
                               timeout=5)
        if result.returncode == 0:
            version = result.stdout.strip()
            logger.info(f"✅ Node.js (nodejs) detected: {version}")
            return True, version
        ...
        logger.error("❌ Node.js check timed out")
        return False, None
    except Exception as e:
        logger.error(f"❌ Error checking Node.js: {e}")
        return False, None

# --- Automatic Package Installation for JavaScript ---
def install_npm_packages(user_folder, message_obj_for_reply):
    """Install npm packages from package.json if exists"""
    package_json_path = os.path.join(user_folder, 'package.json')
    if not os.path.exists(package_json_path):
        return True, "No package.json found"
    
    try:
        bot.reply_to(message_obj_for_reply, "📦 Installing npm dependencies... (This may take a moment)")
        logger.info(f"Installing npm packages in {user_folder}")
        ...
        stdout, stderr = process.communicate(timeout=120)  # 120 second timeout for npm install
        
        if process.returncode == 0:
            logger.info(f"✅ npm install successful in {user_folder}")
            bot.reply_to(message_obj_for_reply, f"✅ npm dependencies installed successfully!")
            return True, "npm dependencies installed successfully"
        else:
            error_msg = f"npm install failed: {stderr[:500]}"
            logger.error(error_msg)
            bot.reply_to(message_obj_for_reply, f"❌ npm install failed: {stderr[:200]}")
            return False, error_msg
            
    except subprocess.TimeoutExpired:
        process.kill()
        logger.error("❌ npm install timed out after 120 seconds")
        bot.reply_to(message_obj_for_reply, "❌ npm install timed out after 120 seconds")
        return False, "npm install timed out"
    except FileNotFoundError:
        logger.error("❌ npm command not found")
        bot.reply_to(message_obj_for_reply, "❌ npm is not installed. Please install Node.js/npm first.")
        return False, "npm is not installed"
    except Exception as e:
        logger.error(f"❌ Error installing npm packages: {e}")
        return False, f"Error installing npm packages: {str(e)}"

def install_specific_npm_module(module_name, user_folder, message_obj_for_reply):
    """Install a specific npm module"""
    try:
        bot.reply_to(message_obj_for_reply, f"🟠 Installing npm package: {module_name}...")
        logger.info(f"Installing npm module {module_name} in {user_folder}")
        ...
        if result.returncode == 0:
            ...
            bot.reply_to(message_obj_for_reply, f"✅ npm package '{module_name}' installed successfully.")
            return True
        else:
            error_msg = f"Failed to install {module_name}: {result.stderr[:500]}"
            logger.error(error_msg)
            bot.reply_to(message_obj_for_reply, f"❌ {error_msg}")
            return False
            
    except subprocess.TimeoutExpired:
        logger.error(f"❌ npm install {module_name} timed out")
        bot.reply_to(message_obj_for_reply, f"❌ npm install {module_name} timed out")
        return False
    except FileNotFoundError:
        logger.error("❌ npm command not found")
        bot.reply_to(message_obj_for_reply, "❌ npm is not installed. Please install Node.js/npm first.")
        ...
        logger.error(f"❌ Error installing npm module {module_name}: {e}")
        bot.reply_to(message_obj_for_reply, f"❌ Error installing npm module: {str(e)}")
        return False
...
# --- JavaScript Script Runner (Improved) ---
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    """Run JavaScript script with proper Node.js handling"""
    max_attempts = 2
    ...
    try:
        ...
        # Check Node.js installation
        node_available, node_version = check_nodejs_installation()
        if not node_available:
            error_msg = ("❌ Node.js is not installed on the server.\n"
                        "Please contact the administrator to install Node.js first.\n"
        ...
        if os.path.exists(package_json_path):
            logger.info(f"📦 package.json found for {script_key}, installing dependencies...")
            success, npm_message = install_npm_packages(user_folder, message_obj_for_reply)
            if not success:
                bot.reply_to(message_obj_for_reply, f"⚠️ npm install had issues: {npm_message}\nTrying to run script anyway...")

        # Syntax check for JavaScript file
        if attempt == 1:
            try:
                logger.info(f"Running JavaScript syntax check for {script_path}")
                ...
                if check_proc.returncode != 0:
                    error_msg = f"❌ JavaScript syntax error in '{file_name}':\n```\n{stderr[:1000]}\n```"
                    logger.error(error_msg)
                    bot.reply_to(message_obj_for_reply, error_msg, parse_mode='Markdown')
                    return
                    
                logger.info(f"✅ JavaScript syntax check passed for {script_key}")
                
            except subprocess.TimeoutExpired:
                logger.warning(f"Syntax check timed out for {script_key}, proceeding anyway...")
                if check_proc:
                    check_proc.kill()
            except FileNotFoundError:
                # This should not happen because we already checked Node.js
                logger.error("❌ Node.js command not found during syntax check")
                bot.reply_to(message_obj_for_reply, "❌ Node.js command not found. Please check Node.js installation.")
                return
            except Exception as e:
                logger.error(f"Error during JavaScript syntax check: {e}")

        # Run the JavaScript file
        logger.info(f"Starting long-running JavaScript process for {script_key}")
        ...
        try:
            ...
            # Use 'node' command (we already verified it exists)
            process = subprocess.Popen(
                ['node', script_path],
                cwd=user_folder,
                env=env,
            ...
            logger.info(f"✅ Started JavaScript process {process.pid} for {script_key}")
            ...
            bot.reply_to(
                ...
                f"✅ JavaScript script '{file_name}' started successfully!\n"
                f"📊 PID: {process.pid}\n"
            ...
            error_msg = f"❌ Error starting JavaScript script '{file_name}': {str(e)}"
            logger.error(error_msg, exc_info=True)
            bot.reply_to(message_obj_for_reply, error_msg)
            
            if process and process.poll() is None:
                logger.warning(f"Killing potentially started JavaScript process {process.pid} for {script_key}")
        ...
        error_msg = f"❌ Unexpected error running JavaScript script '{file_name}': {str(e)}"
        logger.error(error_msg, exc_info=True)
        bot.reply_to(message_obj_for_reply, error_msg)
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if pkg_json:
            logger.info(f"package.json found, checking Node.js...")
            node_available, node_version = check_nodejs_installation()
            if not node_available:
                bot.reply_to(message, "⚠️ Node.js not installed. Cannot install npm packages. JavaScript files may not run.")
            else:
                bot.reply_to(message, f"🔄 Installing Node deps from `{pkg_json}`... (Node.js {node_version})")
                try:
                    command = ['npm', 'install']
                    result = subprocess.run(command, capture_output=True, text=True, check=True, cwd=temp_dir, encoding='utf-8', errors='ignore', timeout=120)
                    logger.info(f"npm install OK. Output:\n{result.stdout}")
                    bot.reply_to(message, f"✅ Node deps from `{pkg_json}` installed.")
                except subprocess.TimeoutExpired:
                    bot.reply_to(message, "❌ npm install timed out after 120 seconds."); return
                except FileNotFoundError:
...
def check_node_command(message):
    """Check Node.js installation status"""
    node_available, node_version = check_nodejs_installation()
    if node_available:
        bot.reply_to(message, f"✅ Node.js is installed!\nVersion: {node_version}")
    else:
        bot.reply_to(message, "❌ Node.js is NOT installed on the server.\nPlease contact administrator.")
...
if __name__ == '__main__':
    ...
    # Check Node.js installation at startup
    node_available, node_version = check_nodejs_installation()
    if node_available:
        logger.info(f"✅ Node.js detected: {node_version}")
    else:
        logger.warning("⚠️ Node.js is not installed. JavaScript files will NOT run!")
